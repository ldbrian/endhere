...
export default function V2ShopkeeperPage() {
  ...
  return (
    <main className="fixed inset-0 z-50 flex flex-col items-center bg-[#050505] font-mono text-zinc-200 selection:bg-zinc-800 selection:text-zinc-100">
      ...
      <header className="absolute left-0 right-0 top-0 z-30 flex h-24 items-center justify-center bg-gradient-to-b from-[#050505] via-[#050505]/80 to-transparent">
        <div className="flex w-full max-w-[430px] items-center justify-between px-8">
          ...
          <span className="text-[10px] tracking-[0.24em] text-zinc-700">SHOPKEEPER ROOM</span>
        </div>
      </header>
      ...
      <div className="relative z-10 flex w-full max-w-[430px] flex-1 flex-col overflow-y-auto px-8 pb-24 pt-28 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div className="mb-10 flex flex-col items-center text-center">
          ...
          <p className="max-w-[22em] text-[11px] leading-relaxed tracking-[0.15em] text-zinc-600">
            一边是此刻的短句，一边是往日留下的观察。
          </p>
        </div>

        <div className="mb-10 grid grid-cols-2 border border-zinc-900 bg-zinc-950/50 p-1">
        ...
        {isLoading ? (
          <p className="mt-10 text-center text-[11px] tracking-widest text-zinc-800">...</p>
        ) : isEmpty ? (
          <p className="mt-10 text-center text-[11px] tracking-widest text-zinc-700">
            这里暂时还是空的。
          </p>
        ) : activeView === 'status' ? (
          <div className="flex flex-col gap-8">
            {statusLogs.map((log, index) => (
              <motion.article
                ...
                className="border-l border-zinc-900 pl-4"
              >
                <time className="mb-3 block text-[9px] tracking-[0.22em] text-zinc-700">
                  {formatLogDate(log.created_at ?? log.updated_at)}
                </time>
                <p className="whitespace-pre-wrap text-[13px] font-light leading-7 tracking-[0.08em] text-zinc-400">
                  {log.value}
                </p>
              </motion.article>
            ))}
          ...
          <div className="flex flex-col gap-8">
            {observations.map((fragment, index) => (
              <motion.article
                ...
                className="border border-zinc-900/90 bg-zinc-950/70 px-6 py-6 shadow-[0_22px_70px_rgba(0,0,0,0.35)]"
              >
                <time className="mb-5 block text-[9px] tracking-[0.22em] text-zinc-700">
        ...
        <div className="mt-14 text-center text-[10px] tracking-[0.3em] text-zinc-800 opacity-60">
          END HERE
        </div>
      </div>
    </main>
