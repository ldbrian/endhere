...
function summarize(rows: VisitLogRow[]) {
  return {
    homeViews: rows.filter((row) => row.event_name === 'v2_home_view').length,
    leaveFragmentTaps: rows.filter((row) => row.event_name === 'v2_leave_fragment_tap').length,
    fragmentNewViews: rows.filter((row) => row.event_name === 'v2_fragment_new_view').length,
    privateFragmentSaves: rows.filter((row) => row.event_name === 'v2_private_fragment_saved').length,
  };
}
