...
function EntryCard({ entry, expanded, setExpanded, formatDate, router, onApplyBandaid }: {
  entry: Entry
      ...
      {expanded === entry.id && !isCurrentlySealed && (
        <div style={{
          ...
          {!entry.released ? (
            ...
            <p style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '12px', opacity: 0.4, letterSpacing: '0.1em' }}>已放下</p>
          )}
        </div>
      )}
    </div>
...
export default function ArchivePage() {
  ...
  const handleApplyBandaid = (id: number, hours: number) => {
    ...
    <div style={{ width: '100%', maxWidth: '360px', display: 'flex', flexDirection: 'column', gap: '24px', padding: '60px 24px', margin: '0 auto' }}>
      ...
      {entries.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {entries.filter(e => !e.released && e.status !== '彻底销毁').map((entry) => (
            <EntryCard key={entry.id} entry={entry} expanded={expanded} setExpanded={setExpanded} formatDate={formatDate} router={router} onApplyBandaid={handleApplyBandaid} />
          ))}

          {entries.some(e => e.released || e.status === '彻底销毁') && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', margin: '8px 0' }}>
              <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.06)' }} />
              <p style={{ color: 'var(--text-muted)', fontSize: '11px', opacity: 0.3, letterSpacing: '0.2em', whiteSpace: 'nowrap' }}>封存的记忆</p>
              <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.06)' }} />
            </div>
          )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {!showFeedback ? (
            <button
              ...
              style={{ width: '100%', padding: '14px', borderRadius: '12px', border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-muted)', fontSize: '12px', letterSpacing: '0.15em', cursor: 'pointer', opacity: 0.5 }}
            >
              你希望 End Here 还能做什么？
            </button>
          ) : (
            <div style={{ padding: '20px', borderRadius: '12px', border: '1px solid var(--border)', background: 'rgba(255,255,255,0.02)', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    ...
                    cursor: 'pointer', transition: 'all 0.2s ease', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '2px',
                  }}
                >
              ...
              <textarea
                ...
                rows={3}
                style={{ width: '100%', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: '10px', padding: '12px 16px', color: 'var(--text-main)', fontSize: '13px', lineHeight: '1.8', fontFamily: 'inherit' }}
              />
              ...
              <button
                ...
                style={{
                  ...
                  border: '1px solid rgba(245,200,66,0.3)', background: 'rgba(245,200,66,0.08)',
                  color: 'var(--warm-yellow)', fontSize: '13px', letterSpacing: '0.15em', cursor: 'pointer',
